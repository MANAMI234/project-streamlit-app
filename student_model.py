...
# --- Page Map ---
page_map = {1: "Home 🏠", 2: "Prediction 📊", 3: "Study Planner", 4: "Review us 💫", 5: "Contact Us 📩"}
...
# --- Prediction Page ---
    ...
    study_hours = st.number_input("Study Hours per Day", min_value=0.0, max_value=24.0, value=0.00)
    attendance = st.number_input("Attendance (%)", min_value=0, max_value=100, value=0)
    previous_grade = st.number_input("Previous Academic Grade (%)", min_value=0, max_value=100, value=0)
    extracurricular = st.selectbox("Co-curricular Participation", ["Select", "Yes", "No"])
    parental_education = st.selectbox("Parent's Education Level", ["Select", "Primary School","Seconday School","High Seconday School","Bachelor's Degree", "Master’s Degree","Diploma", "Honours","Phd"])
